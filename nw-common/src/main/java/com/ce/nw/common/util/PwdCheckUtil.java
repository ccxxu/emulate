...
public class PwdCheckUtil {
	...
	public static String[] KEYBOARD_HORIZONTAL_ARR = {
		"01234567890-=", "!@#$%^&*()_+", "qwertyuiop[]", "QWERTYUIOP{}",
		"asdfghjkl;'", "ASDFGHJKL:", "zxcvbnm,./", "ZXCVBNM<>?",
	};
	
	public static final String INIT_PWD = "a00000000000";
     
	public static String DEFAULT_SPECIAL_CHAR="!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";
	public static String SPECIAL_CHAR = "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";
	
	//复杂（同时包含数字，字母，特殊符号）
	public static final String HIGHT_SPECIAL_CHAR = "^^(?![a-zA-z]+$)(?!\\d+$)(?![!@#$%^&*_-]+$)(?![a-zA-z\\d]+$)(?![a-zA-z!@#$%^&*_-]+$)(?![\\d!@#$%^&*_-]+$)[a-zA-Z\\d!@#$%^&*_-]+$";
	
	//中级（包含字母和数字）
	public static final String MIDDLE_SPECIAL_CHAR = "^(?![a-zA-z]+$)(?!\\d+$)(?![!@#$%^&*]+$)[a-zA-Z\\d!@#$%^&*]+$";
...
//    	boolean bool = checkPassWordComplex("@00000000000");
    	boolean bool = checkPassWordComplex("A00000000000");
    	System.out.println(bool);
    }
    
    public static boolean checkPassWordComplex(String password) {
    	boolean result = true;
    	result = result && checkContainDigit(password);  //判断密码是否包含数字
    	result = result && (checkContainLowerCase(password) || checkContainUpperCase(password));	//判断密码是否包含字母
    	result = result && !INIT_PWD.equals(password.toLowerCase());  //排出初始密码
    	return result;
    }
    ...
    public static boolean checkContainSpecialChar(String password) {
        ...
        for (int i = 0; i < chPass.length; i++) {
            if (MIDDLE_SPECIAL_CHAR.indexOf(chPass[i]) != -1) {
                special_count++;
            }
        }
